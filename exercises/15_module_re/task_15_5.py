...
#Решение
import re
def generate_description_from_cdp(cdp_nei):
	with open(cdp_nei) as f:
		return {i[1]:'description Connected to {} port {}'.format(i[0],i[4]) for i in re.findall('(\S+) +(\S+ \S+) +\d+ + (\S )+ + (\S+) +(\S+ \S+)',f.read())}
